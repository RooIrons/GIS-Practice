...
```{r}
update.packages("tidyverse", repos = "http://cran.us.r-project.org")
update.packages("Rcurl", repos = "http://cran.us.r-project.org")
update.packages("rjson", repos = "http://cran.us.r-project.org")
update.packages("sf", repos = "http://cran.us.r-project.org")
update.packages("rosm", repos = "http://cran.us.r-project.org")
update.packages("ggspatial", repos = "http://cran.us.r-project.org")
```
...
We now have to objects called Latitude and Longitude in the ringing data for the malachite sunbird so that we can map the localities of this species across South Africa. 
...
The purpose of this section is to create a map of southern Africa as a basemap to the localities of malachite sunbirds ringed across South Africa. We want to split these localities according to whether they are considered 'east' or 'west'. This is important because we want to be able to link different moult and breeding patterns in nectarivores across South Africa. Therefore, we need to visually differentiate between east and west so that we can draw conclusions about resource availability and these different patterns. 
...
We could make this map look even better by creating a basemap to these latitude and longitude values. We can then have a clearer picture of how the localities are spread across South Africa. 
```{r step 23}
